    ...
    fun submitFeelingAfterMedication(medication: MedicationEntity, feeling: String?) {
        viewModelScope.launch {
            if (!feeling.isNullOrBlank()) {
                ...
                try {
                    val result = orchestrator.runSymptomAgent(
                        symptoms = listOf("Feeling post-medication"),
                        severity = 1,
                        note = "After taking ${medication.name}: $feeling",
                        patientId = patientId
                    )
                    
                    repo.insertAuditTrail(AgentAuditTrailEntity(
                        patientId = patientId, agentId = "symptom_agent",
                        action = "POST_MED_ANALYSIS", 
                        inputSummary = "Post-med feedback for ${medication.name}: $feeling",
                    ...
                    if (result.escalateImmediately) {
                        showToast("🚨 DANGER: ${result.recommendedAction}")
                    } else if (result.riskLevel == "WARNING") {
                        showToast("⚠️ Warning: ${result.recommendedAction}")
                    } else {
                        showToast("Feedback recorded: ${result.recommendedAction}")
                    }
                } catch (e: Exception) {
                    _ui.update { it.copy(showFeelingDialogFor = null) }
                    // Fallback to manual record if agent fails
                    repo.insertSymptomReport(SymptomReportEntity(
                        patientId = patientId,
                        symptoms = "[\"Feeling post-medication\"]",
                        severity = 1,
                        additionalNote = "After taking ${medication.name}: $feeling",
                        agentRiskLevel = "STABLE",
                        agentReason = "Agent unavailable",
